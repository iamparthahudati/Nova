...
**Goal:** Give Nova persistent local state it can read and write by voice.
**Est. time:** ~2 hrs  
**File:** `nova.py` only. DB file `nova.db` is created automatically on first run.
...
**Done when:**
- `nova.db` is created on first run with all three tables.
...
**Done when:**
- All helpers are callable from a Python shell and produce correct rows in `nova.db`.
...
- "Nova, log task finish the landing page" → task appears in `nova.db`, Nova says "Task logged."
- "Nova, task done finish the landing page" → status flips to `done`.
- "Nova, log earned 2000" → money row inserted.
- "Nova, log progress shipped the new feature" → progress row inserted.
- Restart Nova — all rows are still there.
- "Nova, open Chrome" still works (launcher path unbroken).
