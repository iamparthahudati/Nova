...
- **Keep everything minimal.** Single file (`nova.py`) until a phase explicitly
...
The existing `nova.py`: press Enter, record ~4s with `sounddevice`, transcribe
...
**Files:** modify `nova.py`; create `nova.db` automatically on first run.
...
**Behavior:**
- On startup, create `nova.db` and the tables if they don't exist.
...
- Load secrets at startup via `python-dotenv` (`load_dotenv()` called once at
  the top of `nova.py`). Store them in a `.env` file in the `rai/` folder —
...
**Behavior:**
- LaunchAgent runs `nova.py` at login and keeps it alive.
